...
As the "itemSelection" input returns the label name followed by a dash "-" with a random integer, you can get the cleaned label using `gsub()`, i.e. `gsub(pattern = "\\-[[:digit:]]*$", replacement = "", x = input$itemSelection)`
...
# remove dash "-" followed by a random digit (i.e. MUI tree ID) 
# to return only item label
cleanTreeId <- function(treeId) {
  gsub("\\-[[:digit:]]*$", "", treeId)
}
...
server <- function(input, output, session) {
  output$treeLabel <- shiny::renderPrint(
    if(is.null(input$itemSelection)) {
      cleanTreeId(defaultSelectedId)
    } else { 
      cleanTreeId(input$itemSelection)
    }
  )
}
...
You can use the same Shiny logic with multiple selection.

Use the `selectionPropagation` argument to choose if selecting a parent item does select its children (it doesn't by default).
...
server <- function(input, output, session) {
  ...
  # )
  output$treeLabel <- shiny::renderPrint(
    cleanTreeId(selectedItems$selected)
  )
...
MUI X Tree View is developed MUI, the company behind the popular React framework Material UI. **muiTreeView** is therefore a companion to the R package [**muiMaterial**](https://felixluginbuhl.com/muiMaterial/) , which provides access to all Material UI components from R.
...
You can use `muiMaterial::muiMaterialPage()` and `muiMaterial::CssBaseline()` to avoid using Bootstrap and normalized CSS. See [**muiMaterial**](https://felixluginbuhl.com/muiMaterial/) docs for more information.
